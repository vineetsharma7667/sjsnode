               ...
               StudentData.map(async(item,index)=>{
                   ...
                   try {
                       await Receipt.find({ admission_no }).sort({last_fee_date:1}).exec(async(err,FeesData)=>{
                           ...
                           if(FeesData[0] !=undefined){
                               
                               var A = Moment(FeesData[FeesData.length-1].last_fee_date)
                               var aa =session.split("-")[0];
                               var B= Moment(aa+"-04-01")
                               var last_date 
                               if(parseInt(A.diff(B, 'days')) < 0){
                                 last_date= aa+"-03-31"
                                 try {
                                   var a =session.split("-")[0];
                                   a =a-1
                                   var b =session.split("-")[1];
                                   b=b-1
                                ...
                                //    console.log("previous session "+item.admission_no)
                                 await  DefaulterMaker.find({admission_no,session:previousSession}).exec(async(err,DefaulterData)=>{
                                       ...
                                       previousSessionBalance=parseInt(DefaulterData[0].TotalPreviousBalance)*-1
                                                        ...
                                                        try {
                                                            ...
                                                            await FeeStructure.find({ class_name:class_nameforfees,session }).exec(async(err,FeesClassWise)=>{
                                                                ...
                                                                if(FeesClassWise[0] !=undefined){
                                                                 ...
                                                                 feestructureCount=feestructureCount+1
                                   ...
                                   return res.status(422).send({ error: "error for fetching food DefaulterData" })
                               }
                               }
                               else{
                               

                                 last_date=FeesData[FeesData.length-1].last_fee_date
